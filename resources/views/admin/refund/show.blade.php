...
<div class="max-w-4xl mx-auto pt-5 pb-12 px-4 sm:px-6">
    <div class="mb-8">
        ...
        <p class="text-gray-500 text-xs md:text-sm mt-1">Nomor Pengajuan: <span class="font-mono text-gray-700 font-bold select-all bg-gray-50 px-2 py-0.5 rounded border border-gray-150 break-all">{{ $refund->id }}</span></p>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if($refund->status === 'pending')
                <div class="bg-white p-6 rounded-3xl border border-gray-150 shadow-sm space-y-4">
                    ...
                    <form action="{{ route('admin.refund.action', $refund->id) }}" method="POST" class="space-y-4">
                        @csrf
