...
<div class="max-w-5xl mx-auto pt-4 pb-12">
    <div class="flex items-center gap-6 mb-12">
        <a href="{{ route('admin.blog.show', $blog->id) }}" class="w-12 h-12 rounded-2xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#58CC02] hover:border-[#58CC02] transition-all bg-white shadow-sm">
            <i class="fa-solid fa-arrow-left"></i>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Edit Blog</h1>
        </div>
    </div>

    <form action="{{ route('admin.blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div class="lg:col-span-2 space-y-8">
                    <div>
                        ...
                        <input type="text" name="judulBlog" value="{{ $blog->judulBlog }}" required placeholder="Apa yang ingin Anda bagikan?" class="w-full px-8 py-5 rounded-3xl border border-gray-50 bg-gray-50 outline-none focus:ring-2 focus:ring-[#58CC02] focus:bg-white transition-all font-bold text-xl text-gray-800 placeholder:font-bold placeholder:text-gray-300">
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-3 ml-1">Isi Blog</label>
                        <textarea name="isiBlog" id="editor" rows="15" placeholder="Tuliskan cerita lengkapnya di sini..." class="w-full px-8 py-6 rounded-3xl border border-gray-50 bg-gray-50 outline-none focus:ring-2 focus:ring-[#58CC02] focus:bg-white transition-all text-gray-600 leading-relaxed">{{ $blog->isiBlog }}</textarea>
                    </div>
                </div>
                ...
                <div class="space-y-8">
                    <div class="bg-gray-50 p-8 rounded-4xl border border-gray-100">
                        ...
                        <div class="relative group cursor-pointer">
                            <input type="file" name="fotoBlog" id="fotoInput" class="hidden" accept="image/*">
                            <div onclick="document.getElementById('fotoInput').click()" class="aspect-square bg-white rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-center overflow-hidden">
                                ...
                                <img id="preview"
                                     src="{{ $blog->fotoBlog ? asset('storage/' . $blog->fotoBlog) : '' }}"
                                     class="{{ $blog->fotoBlog ? '' : 'hidden' }} w-full h-full object-cover rounded-xl mb-0">

                                <div id="placeholder" class="{{ $blog->fotoBlog ? 'hidden' : '' }}">
                                    <i class="fa-solid fa-cloud-arrow-up text-3xl text-gray-300 mb-3 group-hover:text-[#58CC02]"></i>
                                    <p class="text-xs font-bold text-gray-400 group-hover:text-[#58CC02]">Ganti Gambar</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-[#58CC02]/5 p-8 rounded-4xl border border-[#58CC02]/10">
                        <h4 class="text-xs font-bold text-[#58CC02] uppercase mb-4 tracking-wider">Status Artikel</h4>
                        <div class="space-y-3">
