...
<div class="flex min-h-screen w-full">
    ...
    <div class="hidden md:flex w-1/2 bg-[#0f8629] flex-col justify-between p-12">
        <div>
            <img src="{{ asset('images/icon.svg') }}" class="w-20">
        </div>

        <div class="flex justify-center items-center flex-1">
            <img src="{{ asset('images/plant.svg') }}" class="w-50">
        </div>
    ...
    <div class="w-full md:w-1/2 flex items-center justify-center relative py-10 overflow-y-auto bg-gray-100 text-sm">
        ...
        <div class="w-full max-w-md px-8">
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="mb-3">
                    <input type="text" name="namaLengkap" value="{{ old('namaLengkap') }}" placeholder="Nama Lengkap" class="w-full bg-transparent border-b border-gray-400 focus:border-[#0f8629] focus:outline-none py-2">
                    @error('namaLengkap')
                ...
                <div class="mb-3">
                    <input type="text" name="noTelp" value="{{ old('noTelp') }}" placeholder="Nomor Telepon" class="w-full bg-transparent border-b border-gray-400 focus:border-[#0f8629] focus:outline-none py-2">
                    @error('noTelp')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Email" class="w-full bg-transparent border-b border-gray-400 focus:border-[#0f8629] focus:outline-none py-2">
                    @error('email')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="text-center text-sm mt-6">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="text-[#0f8629] font-semibold hover:underline">
                    Login
                </a>
            </div>
