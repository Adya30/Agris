        ...
        <div class="px-6 py-4 flex flex-col border-b border-slate-100 shrink-0 bg-white gap-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <a href="<?php echo e(route('admin.produk.index')); ?>" class="w-10 h-10 flex items-center justify-center rounded-full hover:bg-slate-50 text-slate-400 transition-colors">
    ...
    <div :class="activeTarget ? 'flex' : 'hidden md:flex'" class="flex-1 flex-col min-w-0 bg-[#f8fafc] h-full relative overflow-hidden">
        <template v-if="activeTarget">
            ...
            <div class="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 custom-scrollbar bg-[#f8fafc]" id="chat-container">
                <div v-for="chat in chats" :key="chat.id" :class="chat.id_pengirim == <?php echo \Illuminate\Support\Js::from(Auth::id())->toHtml() ?> ? 'flex justify-end' : 'flex justify-start'">
                    <div class="max-w-[85%] md:max-w-[70%] group flex items-start gap-1">
                        ...
                        <div :class="chat.id_penerima == 'GLOBAL' ? 'bg-amber-400 text-amber-950 order-2 rounded-2xl' : (chat.id_pengirim == <?php echo \Illuminate\Support\Js::from(Auth::id())->toHtml() ?> ? 'bg-green-600 text-white rounded-tr-none order-2' : 'bg-white text-slate-700 border-none rounded-tl-none order-2')" class="px-4 py-3 rounded-3xl shadow-sm">
                            ...
                            <div class="flex justify-end items-center gap-1.5 mt-1 text-[9px] font-bold opacity-80">
                                ...
                                <template v-if="chat.id_pengirim == <?php echo \Illuminate\Support\Js::from(Auth::id())->toHtml() ?> && chat.id_penerima !== 'GLOBAL'">
...
<script>
    ...
    createApp({
        setup() {
            const users = ref(<?php echo \Illuminate\Support\Js::from($users)->toHtml() ?>.map(u => ({
                id: u.id,
