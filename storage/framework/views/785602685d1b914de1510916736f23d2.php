<?php $__env->startSection('title', 'Checkout Pembayaran - AGRIS'); ?>

<?php $__env->startSection('content'); ?>
<script src="<?php echo e(config('services.midtrans.is_production', false) ? 'https://app.midtrans.com/snap/snap.js' : 'https://app.sandbox.midtrans.com/snap/snap.js'); ?>" data-client-key="<?php echo e($midtransClientKey); ?>"></script>
...
<div class="max-w-6xl mx-auto pt-6 pb-20 px-4 sm:px-6">
    ...
    <div class="mb-10 text-center sm:text-left flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
            <a href="<?php echo e(route('agen.keranjang.index')); ?>" class="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-400 hover:text-[#58CC02] transition-colors uppercase tracking-widest mb-3">
                        ...
                        <div class="flex items-center gap-4 py-4 first:pt-0 last:pb-0">
                            <div class="w-14 h-14 bg-slate-50 rounded-xl overflow-hidden border border-slate-100 flex items-center justify-center p-1 shrink-0">
                                <?php if($item->produk->fotoProduk): ?>
                                    <img src="<?php echo e(asset('storage/' . $item->produk->fotoProduk)); ?>" class="w-full h-full object-cover rounded-lg shadow-xs">
                                <?php else: ?>
                                    <i class="fa-solid fa-image text-lg text-slate-300"></i>
                                <?php endif; ?>
                            </div>
                            <div class="grow min-w-0">
                                <h4 class="font-extrabold text-slate-800 text-xs truncate"><?php echo e($item->produk->namaProduk); ?></h4>
                                <p class="text-[11px] text-slate-400 mt-1 font-bold">
                                    <?php echo e($item->jumlah); ?> barang • Rp <?php echo e(number_format($item->produk->harga, 0, ',', '.')); ?>
                ...
                <div id="address_section" class="space-y-6">
                    <div class="bg-slate-50/50 p-5 rounded-2xl border border-slate-100 flex items-start gap-4">
                        ...
                        <div class="text-xs leading-relaxed">
                            <p class="font-extrabold text-slate-800 text-sm"><?php echo e($user->namaLengkap); ?> <span class="text-slate-400 font-bold ml-2">• <?php echo e($user->noTelp); ?></span></p>
                            <p class="text-slate-500 mt-1.5"><?php echo e($user->alamatLengkap); ?></p>
                        </div>
                    </div>
            ...
            <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden">
                ...
                <div class="space-y-4 text-xs pb-5 border-b border-slate-100">
                    <div class="flex justify-between text-slate-500 font-bold">
                        ...
                        <span class="text-slate-800 font-extrabold">Rp <?php echo e(number_format($totalPrice, 0, ',', '.')); ?></span>
                    </div>
                    <div class="flex justify-between text-slate-500 font-bold">
                        <span>Total Berat Barang</span>
                        <span class="text-slate-800 font-extrabold"><?php echo e(number_format($totalWeight, 0, ',', '.')); ?> Kg</span>
                    </div>
                    <div class="flex justify-between text-slate-500 font-bold">
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch("<?php echo e(route('agen.checkout.store')); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.modal','data' => ['id' => 'generalErrorModal','title' => 'Kesalahan Sistem','message' => 'Terjadi kesalahan jaringan atau konfigurasi Midtrans key belum valid.','confirmText' => 'Tutup','cancelText' => 'Batal','confirmId' => 'btnCloseErrorModal','cancelId' => 'btnCloseErrorModal2']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['id' => 'generalErrorModal','title' => 'Kesalahan Sistem','message' => 'Terjadi kesalahan jaringan atau konfigurasi Midtrans key belum valid.','confirmText' => 'Tutup','cancelText' => 'Batal','confirmId' => 'btnCloseErrorModal','cancelId' => 'btnCloseErrorModal2']); ?>
