            ...
            <template v-if="activeTarget">
                ...
                <div class="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar bg-[#f8fafc]" id="chat-container">
                    <div v-for="(group, date) in groupedChats" :key="date">
                        ...
                        <div v-for="chat in group" :key="chat.id" :class="chat.id_pengirim == <?php echo \Illuminate\Support\Js::from(Auth::id())->toHtml() ?> ? 'flex justify-end' : 'flex justify-start'" class="mb-4">
                        ...
                        window.Echo.private(`chat.${<?php echo \Illuminate\Support\Js::from(Auth::id())->toHtml() ?>}`).listen('.MessageSent', (e) => {
                            if (e.is_delete) {
                                ...
                                chats.value.forEach(c => {
                                    if (c.id_pengirim == <?php echo \Illuminate\Support\Js::from(Auth::id())->toHtml() ?> && c.id_penerima == e.chat.id_pengirim) c.status = 'dibaca';
                                });
                            } else {
